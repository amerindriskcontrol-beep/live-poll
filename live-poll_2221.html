...
// ---------- storage helpers ----------
const sessionKey = (code) => `session:${code}`;
const responsesKey = (code, pollId) => `responses:${code}:${pollId}`;
...
// ---------- personal poll template library (private — not shared with the audience) ----------
const TEMPLATES_KEY = 'poll_templates';
async function loadTemplates() {
  return (await safeGet(TEMPLATES_KEY, false)) || [];
}
async function saveTemplate(entry) {
  const list = await loadTemplates();
  const next = [...list, entry];
  await safeSet(TEMPLATES_KEY, next, false);
  return next;
}
...
function PollBuilder({ type, onTypeChange, question, setQuestion, options, setOptions, correctAnswer, setCorrectAnswer, onSubmit, submitLabel, onCancel }) {
  ...
  const handleSaveTemplate = async () => {
    ...
    const entry = {
      id: makeId(), name: templateName.trim(), type, question: question.trim(),
...
function Presenter() {
  ...
  const resumeSession = async () => {
    ...
    setResumeError('');
    const sess = await safeGet(sessionKey(c), true);
  ...
  const launch = async () => {
    ...
    const poll = { id: makeId(), type, question: question.trim(), options: finalOptions(type, options), correctAnswer: type === 'mc' ? (correctAnswer || null) : null };
    const sess = { createdAt: Date.now(), active: true, currentIndex: 0, polls: [poll] };
    const ok = await safeSet(sessionKey(newCode), sess, true);
    if (!ok) { setStatus('Session could not be created — please try again.'); return; }
    await safeSet(responsesKey(newCode, poll.id), [], true);
    setCode(newCode);
    setSession(sess);
    setStage('live');
  };
  ...
  const addPollToSeries = async () => {
    const poll = { id: makeId(), type, question: question.trim(), options: finalOptions(type, options), correctAnswer: type === 'mc' ? (correctAnswer || null) : null };
    const latest = (await safeGet(sessionKey(code), true)) || session;
    const updated = { ...latest, polls: [...latest.polls, poll], currentIndex: latest.polls.length };
    await safeSet(sessionKey(code), updated, true);
    await safeSet(responsesKey(code, poll.id), [], true);
    setSession(updated);
    setShowAdd(false);
  ...
  const downloadResults = async () => {
    ...
    for (let i = 0; i < session.polls.length; i++) {
      ...
      const list = (await safeGet(responsesKey(code, p.id), true)) || [];
      if (list.length === 0) {
  ...
  useEffect(() => {
    ...
    const tick = async () => {
      const s = await safeGet(sessionKey(code), true);
      if (s) setSession(s);
      const r = await safeGet(responsesKey(code, currentPoll.id), true);
      if (r) setResponses(r);
    };
    tick();
...
function Audience() {
  ...
  useEffect(() => {
    if (!code) return;
    const tick = async () => {
      const sess = await safeGet(sessionKey(code), true);
  ...
  const submit = async (value) => {
    ...
    const list = (await safeGet(responsesKey(code, currentPoll.id), true)) || [];
    list.push({ id: makeId(), value, ts: Date.now(), votes: 0 });
    await safeSet(responsesKey(code, currentPoll.id), list, true);
